...
impl PipelineHandle {
    ...
    fn set_state(&self, new_state: PipelineState) {
        ...
        if let Some(tray_handle) = self.app_handle.try_state::<crate::TrayHandle>() {
            let tooltip = match new_state {
                PipelineState::Recording => "VoxType - Recording...",
                PipelineState::Transcribing => "VoxType - Transcribing...",
                PipelineState::Polishing => "VoxType - Polishing...",
                PipelineState::Outputting => "VoxType - Outputting...",
                PipelineState::Idle => "VoxType",
            };
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        if let Some(tray_handle) = self.app_handle.try_state::<crate::TrayHandle>() {
            if let Ok(t) = tray_handle.tray.lock() {
                let _ = t.set_tooltip(Some("VoxType - Recording..."));
            }
        }
        crate::refresh_tray(&self.app_handle);
                        ...
                        match chunk {
                            Some(data) => {
                                if let Err(e) = provider.send_audio(&data).await {
                                    tracing::error!("STT send_audio error (audio too long?): {}", e);
    ...
    pub async fn stop(&self) -> Result<()> {
        ...
        if let Some(tray_handle) = self.app_handle.try_state::<crate::TrayHandle>() {
            if let Ok(t) = tray_handle.tray.lock() {
                let _ = t.set_tooltip(Some("VoxType - Transcribing..."));
            }
        }
        crate::refresh_tray(&self.app_handle);
        ...
        *self
        ...
        if raw_text.is_empty() {
            let _ = self
                ...
                .emit(
                    "pipeline:error",
                    "No speech detected. The recording may be too short, or the STT request timed out for long audio. Please try again.",
                );
            self.set_state(PipelineState::Idle);
            return Ok(());
        }
        ...
        if let Some((llm_config, provider)) = pre_llm {
            ...
            match provider.polish(&llm_config, &req, Some(&on_chunk)).await {
                Ok(response) => {
                    // If the LLM returned empty text (e.g. GLM thinking fallback exhausted),
                    // fall back to the raw STT text rather than typing nothing.
                    final_text = if response.polished_text.is_empty() {
                        tracing::warn!(
                            "LLM returned empty polished_text; falling back to raw STT text"
                        );
                        raw_text.clone()
                    } else {
                        ...
                        .await
                    {
                        ...
                        let msg = if e.to_string().to_lowercase().contains("accessibility") {
                            "输出失败：缺少辅助功能权限。请前往「系统设置 → 隐私与安全性 → 辅助功能」，添加并启用 VoxType。".to_string()
                        } else {
                            format!("Output failed: {e}")
                        };
                        let _ = self.app_handle.emit("pipeline:error", msg);
                    }
                }
                Err(e) => {
    ...
    pub async fn pre_warm(&self) {
        ...
        let stt_endpoint = match config.stt_provider.as_str() {
            ...
            "qwen3-asr" => "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions".to_string(),
            _ => {
