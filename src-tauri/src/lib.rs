...
fn build_tray_menu(
    ...
    let about = MenuItem::with_id(app, "about", "About VoxType", true, None::<&str>)?;
...
async fn test_stt_connection(
    ...
    match provider.as_str() {
        ...
        "qwen3-asr" => {
            // Qwen3-ASR uses chat/completions — test via models endpoint
            let client = reqwest::Client::new();
            let resp = client
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let db_path = data_dir.join("voxtype.db");
